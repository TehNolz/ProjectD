...
namespace Webserver.API_Endpoints
{
	...
	internal class Login : APIEndpoint
	{
		...
		public override void POST()
		{
			...
			if (!new Regex("^[A-z0-9]*@[A-z0-9]*.[A-z]*$").IsMatch((string)email) && (string)email != "Administrator")
			{
			...
			var account = User.GetByEmail(Database, (string)email);
			if (account == null)
			{
			...
			var newSession = new Session(Database, account.Email, (bool)rememberMe);
