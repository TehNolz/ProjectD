...
namespace Webserver.Chat
{
	...
	public class Chatroom
	{
		...
		public static List<Chatroom> GetAccessableByUser(SQLiteAdapter database, User user)
		{
			...
			List<Chatroom> result = database.Select<Chatroom>("Private = 0").ToList();

			//Get all private rooms this user can access
			List<Guid> IDs = (from CM in database.Select<ChatroomMembership>("UserID = @ID", new { user.ID }) select CM.ChatroomID).ToList();
